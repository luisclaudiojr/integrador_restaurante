<html>
	<head>
		<title> PROJETO INTEGRADOR - RESTAURANTE </TITLE>
	</head>
	<body>
		<div id='menu'>
			<ul>
				<li><a href="consulta/funcionarios.php">Funcionarios</a></li><!-- AQUI ALTERA OS DADOS DO FUNCIONARIO,EXCLUI  E CADASTRA  -->
				<li><a href="cadastro/cad_usu.php">Usuarios</a></li> <!-- AQUI cadastra os usuarios e as senhas !  -->
				<li><a href="consulta/contas.php">Contas</a></li> <!-- AQUI ALTERA O STATUS, FAZ O FECHAMENTO DA CONTA! ADICIONO MAIS PEDIDOS NA CONTA -->
				<li><a href="consulta/itens.php">Itens</a></li> <!-- AQUI ALTERA OS ITENS, E CADASTRA !  -->
				<li><a href="consulta/mesas.php">Mesas</a></li> <!-- AQUI ALTERA AS MESAS, E CADASTRA !  -->
				
				<li><a href="consulta/faturamento.php">Faturamento</a></li><!-- FATURAMENTO PELO PERIODO EMITE O RELATORIO  DO PERIODO -->
				<li><a href="consulta/pedidos_pendentes.php">PENDENCIAS</a></li><!-- PENDENCIAS - MOSTRA PEDIDO - QTD E MESA - POR ORDEM DE DATA -->
			<ul>
		</div
	</body>
</html>
<?php

	
?>
